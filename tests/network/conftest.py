...
def network_sanity(
    ...
    def _verify_bgp():
        ...
        EXTERNAL_FRR_STATIC_IPV4: reserved IP in CIDR format for the external FRR pod inside VLAN_TAG network.
        ...
        if any(test.get_closest_marker("bgp") for test in collected_tests):
            ...
            required_env_vars = ["VLAN_TAG", "EXTERNAL_FRR_STATIC_IPV4", "BGP_CLUSTER_DOMAIN_GROUP"]
            ...
            if os.environ["BGP_CLUSTER_DOMAIN_GROUP"] not in admin_client.configuration.host:
                failure_msgs.append(
                    f"BGP tests require the valid BGP_CLUSTER_DOMAIN_GROUP='{os.environ['BGP_CLUSTER_DOMAIN_GROUP']} "
                    f"environment variable to match the cluster type. Host: '{admin_client.configuration.host}'"
                )
            else:
